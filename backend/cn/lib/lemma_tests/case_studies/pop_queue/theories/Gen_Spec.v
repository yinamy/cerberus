...
Module Lemma_Defs (P : Parameters).
  ...
  Definition pop_lemma_type : iProp Σ :=
    ∀ (front : Ptr),
∀ (back : Ptr),
∀ (x : Z),
∀ (Q : List),
QueueAux front back Q -∗ ∀ (B : queue_cell),
Owned_queue_cell back B
 -∗ ∃ (Q_post : List),
QueueAux front back Q_post ∗ ∃ (B_post : queue_cell),
Owned_queue_cell back B_post
 ∧ ⌜ (Q = Q_post) ⌝ ∧ ⌜ (B = B_post) ⌝ ∧ (let L := (Snoc (Cons x Q) B.(first)) in ⌜ ((Hd L) = x) ⌝ ∧ ⌜
((Tl L) = (Snoc Q B.(first))) ⌝ ∧ emp).
...
Module Type Lemma_Spec (P : Parameters).
  ...
  Parameter pop_lemma : ⊢ pop_lemma_type.
