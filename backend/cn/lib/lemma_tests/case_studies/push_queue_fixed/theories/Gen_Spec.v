...
Module Defs (P : Parameters).
  ...
  Definition Owned_queue_cell (l: Ptr) (v : queue_cell) : iProp Σ := 
    Owned_int (CN_Lib_Iris.shift l 0 4) v.(first) ∗ 
    padding (arrayshift l 4 1) 4 ∗ 
    Owned_int (CN_Lib_Iris.shift l 8 8) v.(next).
...
(⌜ (f = b) ⌝  ∧  ⌜ ret = (Nil) ⌝) ∨ 
(⌜ (Is_true true) ⌝ ∧ ⌜ ~(f = b) ⌝  ∧ 
∃ (F : queue_cell), Owned_queue_cell f F ∗ 
⌜ (~ (F.(next) = 0)) ⌝ ∗ 
⌜ ((F.(next) = b) ∨ (~ (F.(next) = b))) ⌝ ∗ 
∃ (B : List), QueueAux F.(next) b B ∗ 
⌜ ret = (Cons F.(first) B) ⌝).
...
Module Lemma_Defs (P : Parameters).
  ...
  Definition push_induction_type : iProp Σ :=
    ∀ (front : Ptr), ∀ (second_last : Ptr), ∀ (last : Ptr),

⌜ ((front = second_last) ∨ (~ (front = second_last))) ⌝ -∗ 
  ∀ (Q : List), QueueAux front second_last Q -∗ 
  ∀ (Second_last : queue_cell), Owned_queue_cell second_last Second_last -∗ 
    ⌜ (Second_last.(next) = last) ⌝ -∗ 
    ∀ (Last : queue_cell), Owned_queue_cell last Last -∗ 
    
    ⌜ ((front = last) ∨ (~ (front = last))) ⌝ ∧ 
 ∃ (Q_post : List), QueueAux front last Q_post ∗ 
 ∃ (Last2 : queue_cell), Owned_queue_cell last Last2  ∧ 
...
Module Type Lemma_Spec (P : Parameters).
  ...
  Parameter push_induction : ⊢ push_induction_type.
