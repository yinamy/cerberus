...
Module Defs (P : Parameters).
  ...
  nodes : unsupported ctype array; 
 }.

  Definition Owned_node (l: Ptr) (v : node) : iProp Σ := Owned_int (CN_Lib_Iris.shift l 0 4) v.(v) ∗ padding (arrayshift l 4 1) 4 ∗ unsupported ctype array.*)
...
Module ResourcePredicates (P : Parameters).
  ...
  (*Fixpoint Indirect_Tree (p : Ptr)  (ret : tree) {struct ret}  : iProp Σ := 
(⌜ (Is_true true) ⌝  ∧  Coq_Owned_LAT unsupported BTPtr).

  Fixpoint Tree (p : Ptr)  (ret : (((Z -> tree)) * tree * Z)) {struct ret}  : iProp Σ := 
(⌜ (p = 0) ⌝  ∧  ⌜ ret = ((default_children), (Empty_Tree), (0)) ⌝) ∨ (⌜ (Is_true true) ⌝ ∧ ⌜
~(p = 0) ⌝  ∧ 
∃ (P : node),
Owned_node p P  ∗ let V := P.(v) in  let nodes_ptr := Unsupported member shift in  unsupported resource QPredicate).*)
