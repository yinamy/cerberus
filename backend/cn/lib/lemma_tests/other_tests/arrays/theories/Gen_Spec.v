...
Owned_sllist p H  ∗ ∃ (T : List), SLList_At H.(tail) T ∗ ⌜ ret = (Cons H.(head) T) ⌝).
  ...
  Fixpoint Array (p : Ptr) (n : Z) (ret : List) {struct ret}  : iProp Σ := 
    ...
    (⌜ (Is_true true) ⌝ ∧ ⌜ ~(n = (0)) ⌝  ∧ 
      ∃ (V : Z), Owned_int p V ∗ 
      ∃ (VS : List), 
        Array (CN_Lib_Iris.arrayshift p 4 n) (CN_Lib.wrapI (-2147483648) 2147483647 (n - 1)) VS ∗ 
        ⌜ ret = (Cons V VS) ⌝).
...
∀ (m : Z),
∀ (vs : List),
Array p n vs -∗ ∀ (ws : List),
Array (CN_Lib_Iris.arrayshift p 4 n) m ws -∗ ∃ (xs : List),
Array p (CN_Lib.wrapI (-2147483648) 2147483647 (n + m)) xs ∗ ⌜ (xs = (Append vs ws)) ⌝ ∧ ⌜ Is_true true ⌝.
